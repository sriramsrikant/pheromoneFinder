...
def phyloGroupCandidates(listFile, AsnCAAXFolder):
    ...
    if not(os.path.isdir("PhyloGroup_"+AsnCAAXFolder.split("/")[-1].rstrip("/"))):
        os.mkdir("PhyloGroup_"+AsnCAAXFolder.split("/")[-1].rstrip("/"))
    
    outputFile = os.path.join("PhyloGroup_"+AsnCAAXFolder.split("/")[-1].rstrip("/"), "promoters_0_"listFile.split(".")[0]+".fasta")
    outputHandle = open(outputFile, "w")
