...
def seqIdProcess(seqFolder, dictionaryFile):
    ...
    if not(os.path.isdir(seqFolder) ):
        ...
        seqFileList = [f for f in listdir(seqFolder) if (f.endswith(".fasta") or f.endswith(".fas") or f.endswith(".fa") or f.endswith(".max.pep"))]
        writeCount = 1
        
        ##This is to create the folder to store the processed genome files.
        if not(os.path.isdir(str(seqFolder)+"_Processed")):
            os.mkdir(str(seqFolder)+"_Processed")
        ...
        for g in seqFileList:
            ...
            writeIndex = str(seqFolder)+"_Processed/"+str(g)
        ...
        print "Done writing ", str(writeCount-1), " genome files in the "+str(seqFolder)+"_Processed folder. Good luck!"
